"""Integration tests for the unit listing endpoint.

Units are a global, platform-seeded taxonomy (see ``app/models/units.py``
and the ``a7b8c9d0e1f2_create_unit_and_migrate_item_unit`` migration), so
like categories (and unlike items/suppliers) this endpoint has no
``{business_id}`` path segment. Mirrors ``test_categories.py``.
"""
...
def _build_token(*, permissions: list[str] | None = None) -> str:
    ...
    payload = {
        ...
        "permissions": permissions if permissions is not None else ["inventory.view"],
    }
...
API_PREFIX = "/api/v1/units"


@pytest.mark.asyncio
async def test_list_units_returns_seeded_rows(client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_list_units_sorted_by_sort_order(client: AsyncClient) -> None:
    resp = await client.get(API_PREFIX, headers=AUTH_HEADER)
    assert resp.status_code == 200, resp.text
    sort_orders = [u["sort_order"] for u in resp.json()]
    assert sort_orders == sorted(sort_orders)


@pytest.mark.asyncio
async def test_list_units_excludes_inactive_by_default(client: AsyncClient) -> None:
    resp = await client.get(API_PREFIX, headers=AUTH_HEADER)
    assert resp.status_code == 200, resp.text
...
@pytest.mark.asyncio
async def test_list_units_requires_inventory_view_permission(client: AsyncClient) -> None:
    header = {"Authorization": f"Bearer {_build_token(permissions=[])}"}
    resp = await client.get(API_PREFIX, headers=header)
    assert resp.status_code == 403


@pytest.mark.asyncio
async def test_list_units_returns_abbreviations(client: AsyncClient) -> None:
    resp = await client.get(API_PREFIX, headers=AUTH_HEADER)
    assert resp.status_code == 200, resp.text
    by_code = {u["code"]: u["abbreviation"] for u in resp.json()}
    assert by_code["kg"] == "kg"
    assert by_code["l"] == "L"
    assert by_code["unit"] == "ea"
